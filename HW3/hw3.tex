...
\item (8 pts) In this problem, we will walk through a signle step of the gradient descent algorithm for logistic regression. Assume two dimension input.
Recap:
$$f(x;w, b) = \sigma(w \cdot x + b)$$
$$\text{Cross entropy loss } L(\hat{y}, y) = -[y \log  \hat{y} + (1-y)\log(1-\hat{y})]$$
$$\text{The single update step } \theta^{t+1} = \theta^{t} - \eta \nabla_{\theta} L(f(x;\theta), y),\text{ where } \theta=[w_{1}, w_{2}, b]^T $$

Now given
$$ \text{Initial parameters : } w_{1}=w_{2}=b=0,(\Rightarrow \theta^{0}=[0, 0, 0]))$$
...
\begin{enumerate}
	...
	\item (4 pts) Compute the updated parameter vector $\theta^{1}$ from the single update step.
...
\begin{enumerate}
	...
	\item (10 pts) Implement 1NN, Run 5-fold cross validation. Report accuracy, precision, and recall in each fold.
	
	\begin{soln}  Solution goes here. \end{soln}
	
	\item (10 pts) Implement logistic regression, Run 5-fold cross validation. Report accuracy, precision, and recall in each fold.
